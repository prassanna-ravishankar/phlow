-- Phlowtop Database Schema Extensions
-- This adds monitoring tables to track agent lifecycle and task execution

-- Extend existing agent_cards table for monitoring
...
CREATE INDEX IF NOT EXISTS idx_phlow_tasks_agent_id ON phlow_tasks(agent_id);
CREATE INDEX IF NOT EXISTS idx_phlow_tasks_status ON phlow_tasks(status);
CREATE INDEX IF NOT EXISTS idx_phlow_tasks_created_at ON phlow_tasks(created_at);
CREATE INDEX IF NOT EXISTS idx_phlow_messages_task_id ON phlow_messages(task_id);
CREATE INDEX IF NOT EXISTS idx_phlow_messages_created_at ON phlow_messages(created_at);

-- Enable Row Level Security
ALTER TABLE phlow_tasks ENABLE ROW LEVEL SECURITY;
ALTER TABLE phlow_messages ENABLE ROW LEVEL SECURITY;

-- RLS policies for phlow_tasks
-- Allow agents to read all tasks (for monitoring)
CREATE POLICY phlow_tasks_read ON phlow_tasks
    FOR SELECT USING (true);

-- Allow agents to create and update their own tasks
CREATE POLICY phlow_tasks_own ON phlow_tasks
    FOR ALL USING (
        agent_id = current_setting('phlow.agent_id', true)
        OR client_agent_id = current_setting('phlow.agent_id', true)
    );

-- RLS policies for phlow_messages
-- Allow agents to read all messages (for monitoring)
CREATE POLICY phlow_messages_read ON phlow_messages
    FOR SELECT USING (true);

-- Allow agents to insert messages for tasks they're involved in
CREATE POLICY phlow_messages_write ON phlow_messages
    FOR INSERT WITH CHECK (
        source_agent_id = current_setting('phlow.agent_id', true)
        OR target_agent_id = current_setting('phlow.agent_id', true)
    );

-- Function to increment active tasks
...
-- Function to decrement active tasks
...
-- Trigger to auto-update the updated_at column for phlow_tasks
...
-- Note: This requires SUPERUSER privileges. If not available, ask DB admin to run:
-- ALTER PUBLICATION supabase_realtime ADD TABLE agent_cards, phlow_tasks, phlow_messages;

-- View for agent monitoring summary
