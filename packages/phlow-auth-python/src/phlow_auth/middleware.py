...
from .types import PhlowConfig, PhlowContext, VerifyOptions, AgentCard
from .jwt_utils import verify_token, is_token_expired
...
class PhlowMiddleware:
    ...
    async def authenticate(
        ...
        if self.rate_limiter and not self.rate_limiter.is_allowed(agent_id):
            await self._log_audit("auth_failure", agent_id, details={"reason": "rate_limit"})
            raise RateLimitError("Rate limit exceeded")
        
        # Get agent card from database
        agent_card = await self._get_agent_card(agent_id)
        if not agent_card:
            await self._log_audit("auth_failure", agent_id, details={"reason": "agent_not_found"})
        ...
        if options.required_permissions:
            missing_permissions = set(options.required_permissions) - set(claims.permissions)
