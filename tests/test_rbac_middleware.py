...
class TestPhlowMiddlewareRBAC:
    ...
    def middleware(self, mock_config):
        ...
        with patch('phlow.middleware.create_client'), \
    ...
    async def test_authenticate_with_role_cached_valid(self, middleware, valid_token):
        ...
        with patch.object(middleware, 'verify_token') as mock_verify:
            ...
            middleware.role_cache.get_cached_role.assert_called_once_with("test-agent-123", "admin")
    ...
    async def test_authenticate_with_role_cache_expired(self, middleware, valid_token):
        ...
        with patch.object(middleware, 'verify_token') as mock_verify, \
             patch.object(middleware, '_send_role_credential_request', return_value=mock_response):

            mock_context = MagicMock()
            mock_context.agent.metadata = {"agent_id": "test-agent-123"}
            mock_verify.return_value = mock_context

            with pytest.raises(AuthenticationError, match="Role credential request failed"):
                await middleware.authenticate_with_role(valid_token, "admin")

    @pytest.mark.asyncio
    async def test_authenticate_with_role_successful_verification(self, middleware, valid_token):
        ...
        middleware.role_verifier.verify_presentation = AsyncMock(return_value=verification_result)
    ...
    @pytest.mark.asyncio
    async def test_authenticate_with_role_verification_failed(self, middleware, valid_token):
        ...
        with patch.object(middleware, 'verify_token') as mock_verify, \
            ...
            with pytest.raises(AuthenticationError, match="Role credential verification failed"):
                await middleware.authenticate_with_role(valid_token, "admin")
    ...
    async def test_authenticate_with_role_no_response(self, middleware, valid_token):
        ...
        with patch.object(middleware, 'verify_token') as mock_verify, \
             patch.object(middleware, '_send_role_credential_request', return_value=None):
